    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (isInteger(input[0])) val1 = Integer.parseInt(input[0]);
                ...
                val1 = RomanNums.romanToArabic(input[0]);
                isArabic = false;
            }
            ...
            if (isInteger(input[2])) {
                if (!isArabic)
                    throw new RuntimeException("Вы ввели разные типы чисел, не надо так :(");
                val2 = Integer.parseInt(input[2]);
            } else {
                if (isArabic)
                    throw new RuntimeException("Вы ввели разные типы чисел, не надо так :(");
                val2 = RomanNums.romanToArabic(input[2]);
            }

            if (val1 < 1 || val1 > 10 || val2 < 1 || val2 > 10)
                throw new RuntimeException("Числа должны быть от 1 до 10 включительно!");

            System.out.println(CalculationProcess.getCalculationResult(val1, val2, operation));
        }
